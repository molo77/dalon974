    ...
    if (activeTab === 'scraper') {
      ...
      const checkPausedRuns = () => {
        const pausedRun = scraperRuns.find(run => 
          run.status === 'paused' && 
          new Date(run.startedAt).getTime() > Date.now() - 300000 // Dans les 5 dernières minutes
        );
        ...
        <div className='space-y-8'>
          <div className='flex items-center justify-between'>
            <div className='flex items-center gap-4'>
              ...
              {scraperRuns.some(run => run.status === 'paused' && new Date(run.startedAt).getTime() > Date.now() - 300000) && (
