...
console.log('🔧 Configuration initiale du serveur Linux pour Dalon974 (sans sudo interactif)');
...
try {
  ...
  // 3. Configurer sudo sans mot de passe pour l'utilisateur
  console.log('🔐 Configuration de sudo sans mot de passe...');
  const sudoConfig = `molo ALL=(ALL) NOPASSWD: ALL`;
  execSync(`ssh ${SERVER_HOST} "echo '${sudoConfig}' | sudo tee /etc/sudoers.d/molo"`, { stdio: 'inherit' });
...
if [ $? -ne 0 ]; then
  ...
  // 5. Créer un service systemd simple
...
WantedBy=multi-user.target`;
  ...
  execSync(`ssh ${SERVER_HOST} "echo '${systemdService}' | sudo tee /etc/systemd/system/rodcoloc.service > /dev/null"`, { stdio: 'inherit' });
