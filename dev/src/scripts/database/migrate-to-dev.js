// Migration complète vers rodcoloc_dev
const { execSync } = require('child_process');
const path = require('path');

async function migrateToDev() {
  console.log('🚀 Migration complète vers rodcoloc_dev\n');
  console.log('📋 Étapes de la migration:');
  console.log('   1. Export des données actuelles');
  console.log('   2. Création de la base rodcoloc_dev');
  ...
  try {
    ...
    execSync('node src/scripts/database/export-database.js', { 
    ...
    console.log('🏗️ ÉTAPE 2: Création de la base rodcoloc_dev...');
    execSync('node src/scripts/database/create-dev-database.js', { 
      cwd: path.join(__dirname, '../../..'),
      stdio: 'inherit'
    });
    ...
    execSync('node src/scripts/database/apply-schema-to-dev.js', { 
      cwd: path.join(__dirname, '../../..'),
      stdio: 'inherit'
    });
    ...
    execSync('node src/scripts/database/import-database.js', { 
      cwd: path.join(__dirname, '../../..'),
      stdio: 'inherit'
    });
